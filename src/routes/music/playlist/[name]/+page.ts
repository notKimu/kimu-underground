...
export async function load({ params }) {
  const playlistData = await import(`$lib/data/json/${params.name}-playlist-info.json`).catch(() => {
    error(404, "Playlist not found.");
  }) as unknown as PlaylistData;
  const playlistTracks = await import(`$lib/data/json/${params.name}-playlist.json`).catch(() => {
    error(404, "Playlist not found.");
